...
class TraineeController {
	...
	public async getAll(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const traineeData = await traineeRepository.findData({ deletedAt: undefined, skip, limit, search });
	...
	public async login(req: Request, res: Response, next: NextFunction) {
		try {
			...
			return res.status(400).send({ status: 400, error: error, message: 'trainee Id or Password is Invalid' });
		}
	}
}
